...
public class Booksdatabase
{
    ...
    {
        if (File.Exists("books.sqlite"))
        {
            try
            {
                SQLiteConnection connection =
                    new SQLiteConnection(
                    "Data Source=books.sqlite;Version=3;New=False;Compress=True;");
                connection.Open();
                ...
                {
                    book tempBook;
                    tempBook.title = Convert.ToString(data[0]);
                    tempBook.author = Convert.ToString(data[1]);
                    tempBook.location = Convert.ToString(data[2]);
                    texts[nElement] = tempBook;
                    nElement++;
                }
                connection.Close();
            }
    ...
    public static void Save()
    {
        try
        {
            SQLiteConnection connection =
                new SQLiteConnection(
                "Data Source=books.sqlite;Version=3;New=True;Compress=True;");
            connection.Open();
