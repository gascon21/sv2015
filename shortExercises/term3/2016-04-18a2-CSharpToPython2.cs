// Basic C# to Python converter, second approach

using System;
using System.IO;

public class CSharpToPhyton2
{
        ...
        do
        {
            ...
            if (line != null)
            {
                ...
                if (line.Contains("for (") || line.Contains("for("))
                {
                    ...
                    int variableStart = line.IndexOf("int ") + 4;
                    int variableEnd = line.IndexOf("=");
                    int firstSemiColon = line.IndexOf(";");
                    int comparator = line.IndexOf("<");
                    int secondSemiColon = line.IndexOf(";", firstSemiColon+1);
                    string spaces = line.Substring(0, line.IndexOf("for"));
                    string variableName = line.Substring(variableStart,
                        variableEnd - variableStart);
                    string startValue = line.Substring(variableEnd+1,
                        firstSemiColon - variableEnd-1);
                    string endValue = line.Substring( comparator+1,
                        secondSemiColon- comparator-1);
                    line = spaces + "for " + variableName + " in range ("+
                        startValue + "," +endValue + "):";
                }
