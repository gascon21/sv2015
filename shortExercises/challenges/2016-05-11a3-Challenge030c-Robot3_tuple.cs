...
// Version using "tuples" (.Net V4 or above)
#define debugging
...
public class TreasureSearch
{
    public static void Main()
    {
        ...
        Queue<Tuple<int,int>> candidates = new Queue<Tuple<int,int>>();
        ...
        int startX, int startY)
    {
        ...
        map[startX, startY] = 'X';
        candidates.Enqueue(new Tuple<int,int>(startX+1, startY));
        candidates.Enqueue(new Tuple<int,int>(startX-1, startY));
        candidates.Enqueue(new Tuple<int,int>(startX, startY+1));
        candidates.Enqueue(new Tuple<int,int>(startX,startY-1));

        while( candidates.Count > 0)
        {
            Tuple < int,int> next = candidates.Dequeue();
            if (Reachable(map, width, height, 
                    candidates, next.Item1, next.Item2))
                return true;
        }
        return false;
    }
